...
class Members extends Component
{
    ...
    {
        $pageName = ucfirst($this->subModule);
        ...
        if($this->search == null) {
            if($this->tab == 'index') {  
                ...
                if($this->sortBy) {
                    ...
                    $users = User::isMember()->orderBy($expSort[0],$expSort[1])->paginate($showDataTotal);
                } else {
                    $users = User::isMember()->descending()->paginate($showDataTotal);
                }

            } else {
                if($this->sortBy) {
                    $expSort = explode('-',$this->sortBy);
                    $users = User::isMember()->onlyTrashed()->orderBy($expSort[0],$expSort[1])->paginate($showDataTotal);
                } else {
                    $users = User::isMember()->onlyTrashed()->descending()->paginate($showDataTotal);
                }
        ...
        return view('livewire.backend.members.index',compact('pageName',$this->module));
    ...
    public function store(Request $request)
    {
        ...
        $this->userId ? $this->emit('alert', ['type' => 'success', 'message' => __('validation.action.updated',array('attribute' => $this->subModule))]) : $this->emit('alert', ['type' => 'success', 'message' => __('validation.action.created',array('attribute' => $this->subModule))]);
    ...
    public function updatePassword(Request $request)
    {
        ...
        $this->emit('alert', ['type' => 'success', 'message' => __('validation.action.password',array('attribute' => $this->subModule))]);
    ...
    public function restore($id)
    {
        ...
        $this->emit('alert', ['type' => 'success', 'message' => __('validation.action.restored',array('attribute' => $this->subModule))]);
    }
    ...
    {
        User::find($id)->delete();
        $this->emit('alert', ['type' => 'warning', 'message' => __('validation.action.archived',array('attribute' => $this->subModule))]);
    }
    ...
    public function destroy($id)
    {
        ...
        $users->forceDelete();
        $this->emit('alert', ['type' => 'error', 'message' => __('validation.action.destroyed',array('attribute' => $this->subModule))]);
    }
