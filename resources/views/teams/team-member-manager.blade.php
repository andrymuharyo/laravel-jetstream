<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-form-section submit="addTeamMember">
                <x-slot name="title">
                    {{ __('team.label.member.name') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('team.label.member.info') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600">
                            {{ __('team.label.member.email') }}
                        </div>
                    </div>
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            <x-jet-label for="role" value="{{ __('label.role.name') }}" />
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('team.label.member.lists.name') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('team.label.member.lists.info') }}
                </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingLeavingTeam">
        <x-slot name="title">
            {{ __('team.alert.leave_team.name') }}
        </x-slot>

        <x-slot name="content">
            {{ __('team.alert.leave_team.info') }}
        </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingTeamMemberRemoval">
        <x-slot name="title">
            {{ __('team.alert.remove_team.name') }}
        </x-slot>

        <x-slot name="content">
            {{ __('team.alert.remove_team.info') }}
        </x-slot>
