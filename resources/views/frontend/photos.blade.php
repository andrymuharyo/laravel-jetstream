...
@push('bottom.script')
    <script>
        $('select[name="photo_category"]').on('change',function() {
    ...
    <div class="container">
        <div class="landing-overview base-padding">
            <div class="row">
                <div class="col-md-8">
                    <div class="content">
                        <h1 class="title">{{ __('menu.photos.name') }}</h1>
                        <div class="media-container">
                            <form class="form-inline">
                                ...
                                <div class="form-group mx-sm-3 mb-2">
                                    <select class="form-control form-control-sm js-choices" name="photo_category">
                                        @foreach($photoCategories as $photoCategory)
                                            <option @if(request('category') && $photoCategory->id == request('category')) selected @endif value="{{ $photoCategory->id }}">{{ $photoCategory->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </form>
                            <div class="row">
                                ...
                                @foreach($photos as $photo)
                                    <div class="col-md-6">
                                        <div class="media">
                                            <a class="media-content" href="{{ route('frontend.photos.view',$photo->id) }}">
                                                @if($photo && $photo->hasImage('image'))
                                                    <div class="media-image" style="background-image: url({{ $photo->getImage('image') }});background-size: 100% 250px;background-repeat:no-repeat;">
                                                        <img src="{{ asset('frontend/images/news-thumb.png') }}" alt="{{ $photo->caption }}">
                                                    </div>
                                                @endif
