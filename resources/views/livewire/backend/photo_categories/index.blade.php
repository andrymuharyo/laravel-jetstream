            ...
            @if(count($photoCategories) > 0)
                <table class="table-fixed w-full border-2 border-gray-200 sm:border-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ $photoCategory->no }}</td>
                            <td class="border-b-2 border-gray-100 px-4 py-2">{{ $photoCategory->title }}</td>
                            <td class="border-b-2 border-gray-100 px-4 py-2">
                                @if($photoCategory->active)
                            ...
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ ($photoCategory->submitted_at) ? $photoCategory->submitted_at->toFormattedDateString() : null }}</td>
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                {{ ($photoCategory->created_at) ? $photoCategory->created_at->toFormattedDateString() : null }}
                                <br/>
                                {{ ($photoCategory->updated_at) ? $photoCategory->updated_at->toFormattedDateString() : null }}
                            </td>
                            <td class="border-b-2 border-gray-100 px-4 py-2 text-center">
                                @include('components.backend.action',array('id' => $photoCategory->id))
                            </td>
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                <form method="GET" action="{{ route('backend.'.mb_strtolower(str_replace('_categories','s',$pageName))) }}">
                                    ...
                                    <input type="hidden" name="category" value="{{ $photoCategory->id }}">
                                </form>
                            </td>
                        </tr>
                        @endforeach
                ...
                @if($photoCategories->hasPages())
                    <div class="grid sm:grid-flow-col gap-4 mt-5">
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            {!! $photoCategories->withQueryString()->links() !!}
                        </div>
                    </div>
                @endif
            @else
