                ...
                @if(count($links) > 0)
                    <div wire:sortable="reOrder" wire:sortable-group="reOrder" class="overflow-hidden">
                        @foreach($links as $key => $link)
                            <section class="mb-4 draggable-section" wire:key="group-{{ $link->id }}" wire:sortable.item="{{ $link->id }}">
                                    ...
                                    <div class="relative py-3 px-5">
                                        <div class="grid grid-cols-12 gap-2">
                                            ...
                                            <div class="col-span-12 sm:col-span-4">
                                                <p class="overflow-ellipsis overflow-hidden truncate">{{ $link->title }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <span class="absolute top-2 right-2">
                                            @include('components.backend.action-nestable',array('id' => $link->id,'create' => false))
                                        </span>
                                    </div>
                                </div>
                            </section>
