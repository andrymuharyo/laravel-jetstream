                ...
                @if(count($contents) > 0)
                    <table class="table-fixed w-full border-2 border-gray-200 sm:border-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ $content->no }}</td>{{-- 
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                    @if($content->image && $content->hasImage('image'))
                                        <div class="bg-cover bg-center bg-auto border-solid border-white border-2 shadow-md w-full h-20" style="background-image:url({{ $content->getImage('image') }});"></div>
                                    @else
                                ...
                                <td class="border-b-2 border-gray-100 px-4 py-2">{{ $content->title }}</td>
                                ...
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ ($content->submitted_at) ? $content->submitted_at->toFormattedDateString() : null }}</td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                    {{ ($content->created_at) ? $content->created_at->toFormattedDateString() : null }}
                                    <br/>
                                    {{ ($content->updated_at) ? $content->updated_at->toFormattedDateString() : null }}
                                </td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 text-center">
                                    @include('components.backend.action',array('id' => $content->id))
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
