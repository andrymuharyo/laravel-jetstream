                ...
                @if(count($slides) > 0)
                    <table class="table-fixed w-full border-2 border-gray-200 sm:border-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ $slide->no }}</td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                    @if($slide->image && $slide->hasImage('image'))
                                        <div class="bg-cover bg-center bg-auto border-solid border-white border-2 shadow-md w-full h-20" style="background-image:url({{ $slide->getImage('image') }});"></div>
                                    @else
                                ...
                                <td class="border-b-2 border-gray-100 px-4 py-2">{{ $slide->title }}</td>
                                ...
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ ($slide->submitted_at) ? $slide->submitted_at->toFormattedDateString() : null }}</td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                    {{ ($slide->created_at) ? $slide->created_at->toFormattedDateString() : null }}
                                    <br/>
                                    {{ ($slide->updated_at) ? $slide->updated_at->toFormattedDateString() : null }}
                                </td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 text-center">
                                    @include('components.backend.action',array('id' => $slide->id))
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
