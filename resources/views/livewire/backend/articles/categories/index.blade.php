...
@if($isForm)
    @include('livewire.backend.articles.categories.form')
@else
<x-slot name="header">
            ...
            @if(count($articleCategories) > 0)
                <table class="table-fixed w-full border-2 border-gray-200 sm:border-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ $articleCategory->no }}</td>
                            <td class="border-b-2 border-gray-100 px-4 py-2">{{ $articleCategory->title }}</td>
                            ...
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ ($articleCategory->submitted_at) ? $articleCategory->submitted_at->toFormattedDateString() : null }}</td>
                            <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                {{ ($articleCategory->created_at) ? $articleCategory->created_at->toFormattedDateString() : null }}
                                <br/>
                                {{ ($articleCategory->updated_at) ? $articleCategory->updated_at->toFormattedDateString() : null }}
                            </td>
                            <td class="border-b-2 border-gray-100 px-4 py-2 text-center">
                                @include('components.backend.action',array('id' => $articleCategory->id))
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                @if($articleCategories->hasPages())
                    <div class="grid sm:grid-flow-col gap-4 mt-5">
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            {!! $articleCategories->withQueryString()->links() !!}
                        </div>
                    </div>
                @endif
            @else
