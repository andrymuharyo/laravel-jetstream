                ...
                @if(count($builders) > 0)
                    <table class="table-fixed w-full border-2 border-gray-200 sm:border-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ $builder->no }}</td>
                                <td class="border-b-2 border-gray-100 px-4 py-2">{{ $builder->title }}</td>
                                ...
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">{{ ($builder->submitted_at) ? $builder->submitted_at->toFormattedDateString() : null }}</td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 hidden sm:table-cell">
                                    {{ ($builder->created_at) ? $builder->created_at->toFormattedDateString() : null }}
                                    <br/>
                                    {{ ($builder->updated_at) ? $builder->updated_at->toFormattedDateString() : null }}
                                </td>
                                <td class="border-b-2 border-gray-100 px-4 py-2 text-center">
                                    @include('components.backend.action',array('id' => $builder->id))
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    @if($builders->hasPages())
                        <div class="grid sm:grid-flow-col gap-4 mt-5">
                            ...
                            <div class="col-span-12 sm:col-span-6">
                                {!! $builders->withQueryString()->links() !!}
                            </div>
                        </div>
                    @endif
                @else
