    ...
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
        <div class="shadow overflow-hidden sm:rounded-md">
            ...
            <div class="px-2 py-4 bg-white sm:px-6">
                @if(count($navigations) > 0)
                    <div wire:sortable="reOrder" wire:sortable-group="reOrderGroup" class="overflow-hidden">
                        @foreach($navigations as $key => $navigation)
                            <section class="draggable-section mb-4" wire:key="{{ $navigation->id }}" wire:sortable.item="{{ $navigation->id }}">
                                <div class="draggable-item bg-gray-100 rounded-md shadow">
                                    <div class="relative py-3 px-5">
                                        ...
                                        <span class="absolute top-2 right-2">
                                            @include('components.backend.action-nestable',array('id' => $navigation->id,'create' => true))
                                        </span>
                                    </div>
                                </div>
                            </section>
                            @if(count($navigation->children()) <> 0)
                                <div class="ml-10 {{ $this->isCollapse ? 'hidden' : '' }}" wire:sortable-group.item-group="{{ $navigation->id }}">
                                    @foreach ($navigation->children() as $navigationChild)
                                        <section class="mb-4" wire:key="{{ $navigationChild->id }}" wire:sortable-group.item="{{ $navigationChild->id }}">
                                            <div class="draggable-item bg-gray-100 rounded-md shadow">
                                                <div class="relative py-3 px-5">
                                                    <div class="grid grid-cols-12 gap-2">
                                                        <div wire:sortable.handle class="col-span-12 sm:col-span-1 text-gray-800 cursor-move">
                                                    ...
                                                    <span class="absolute top-2 right-2">
                                                        @include('components.backend.action-nestable',array('id' => $navigationChild->id,'create' => false))
                                                    </span>
                                                </div>
                                            </div>
                                        </section>
